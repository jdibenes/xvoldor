...
namespace cvl {
	...
	template<class T> 
	__mlib_host_device
	inline bool root2real(T b, T c, T& r1, T& r2) {
...
#define ADD_CUBIC_COUNT_SAMPLE(sample)
	...
	*/
	...
	template<> 
	__mlib_host_device
	constexpr double get_numeric_limit<double>() {
		// abs limit is 17 digits
		return 1e-13;
	}
	template<> 
	__mlib_host_device
	constexpr long double get_numeric_limit<long double>() {
