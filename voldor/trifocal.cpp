...
)
{
    ...
    mn(Eigen::indexing::all, Ind2) = p_in(Eigen::seq(11, 16), 0).reshaped(3, 2);

    // % tensor
    // T1=(S(:,1)*e21.'+e31*mn(1,:)).';
    Eigen::Matrix<float, 3, 3> T1 = (S(Eigen::indexing::all, 0) * e21.transpose() + e31 * mn(0, Eigen::indexing::all)).transpose();
    
    // T2=(S(:,2)*e21.'+e31*mn(2,:)).';
    Eigen::Matrix<float, 3, 3> T2 = (S(Eigen::indexing::all, 1) * e21.transpose() + e31 * mn(1, Eigen::indexing::all)).transpose();
    
    // T3=(S(:,3)*e21.'+e31*mn(3,:)).';
    Eigen::Matrix<float, 3, 3> T3 = (S(Eigen::indexing::all, 2) * e21.transpose() + e31 * mn(2, Eigen::indexing::all)).transpose();

    // % other slices of the TFT
    // J3=[T1(3,:);T2(3,:);T3(3,:)];
    Eigen::Matrix<float, 3, 3> J3;
    J3 << T1(2, Eigen::indexing::all), T2(2, Eigen::indexing::all), T3(2, Eigen::indexing::all);

    // K3=[T1(:,3),T2(:,3),T3(:,3)];
    Eigen::Matrix<float, 3, 3> K3;    
    K3 << T1(Eigen::indexing::all, 2), T2(Eigen::indexing::all, 2), T3(Eigen::indexing::all, 2);
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        Ap(Eigen::seq(ind2 + 0, ind2 + 3), Eigen::indexing::all) = Eigen::kroneckerProduct(S3, S2).transpose() * Eigen::kroneckerProduct(x1.colwise().homogeneous().transpose(), I9);
    ...
    D(Eigen::indexing::all, Eigen::seq(0, 8)) = Eigen::kroneckerProduct(I3, Eigen::kroneckerProduct(I3, e21));
    
    // aux=zeros(3,2); 
    // aux(Ind2,:)=eye(2);
    Eigen::Matrix<float, 3, 2> aux = Eigen::Matrix<float, 3, 2>::Zero();
    aux(Ind2, Eigen::indexing::all) = I2;
    ...
    D(Eigen::indexing::all, Eigen::seq(9, 10)) = tmp_D_9_10;

    // D(:,12:14)=kron(eye(3),kron(e31,aux(:,1)));
    D(Eigen::indexing::all, Eigen::seq(11, 13)) = Eigen::kroneckerProduct(I3, Eigen::kroneckerProduct(e31, aux.col(0)));
    
    // D(:,15:17)=kron(eye(3),kron(e31,aux(:,2)));
    D(Eigen::indexing::all, Eigen::seq(14, 16)) = Eigen::kroneckerProduct(I3, Eigen::kroneckerProduct(e31, aux.col(1)));
    ...
    D(Eigen::indexing::all, Eigen::seq(17, 19)) = tmp_D_17_19;
...
)
{
    ...
    for (it = 0; it < it_max; ++it)
    {
        ...
        // dt=aux(1:u,:); 
        // dy=aux(u+1:u+s,:);
        dt = aux(Eigen::seq(0, u - 1), Eigen::indexing::all);
        dy = aux(Eigen::seq(u, u + s - 1), Eigen::indexing::all);
...
)
{
   ...
   // std::cout << Ind << std::endl;
    ...
    Eigen::Matrix<float, 3, 3> R2 = rpt.v2.P(Eigen::indexing::all, Eigen::seqN(0, 3));
    Eigen::Matrix<float, 3, 3> R3 = rpt.v3.P(Eigen::indexing::all, Eigen::seqN(0, 3));
