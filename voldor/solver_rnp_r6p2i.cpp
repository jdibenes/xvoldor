...
bool solver_r6p2i(float const* p3d_1, float const* p2d_2, bool direction, float r0, float* r_12, float* t_12, float* dr_12, float* dt_12, int max_iterations)
{
	...
	bool ig = cv::solvePnP({ reinterpret_cast<cv::Point3f const*>(p3d_1), 4 }, { reinterpret_cast<cv::Point2f const*>(p2d_2), 4 }, cv::Mat::eye(3, 3, CV_32FC1), cv::Mat(), r_initial, t_initial, false, cv::SOLVEPNP_AP3P);
	if (!ig) { return false; }
