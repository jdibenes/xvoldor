...
static void weight_points(const Eigen::Matrix<double, 1, 3> & z, const Eigen::Matrix<double, 2, 4> & x, Eigen::Matrix<double, 2, 4> * xp)
{
    xp->col(0) = x.col(0);
...
static void centering_rotation(const Eigen::Vector3d& x0, Eigen::Matrix3d* R)
{
...
static double solver_4p3v_para(const Eigen::Matrix<double, 2, 4> &x1, const Eigen::Matrix<double, 2, 4> &x2, const Eigen::Matrix<double, 2, 4> &x3, Solution *sol)
{
...
static void solve_for_translation(const Eigen::Matrix<double, 2, 4> &x1, const Eigen::Matrix<double, 2, 4> &x2, const Eigen::Matrix<double, 2, 4> &x3, const Eigen::Matrix3d &R2, const Eigen::Matrix3d &R3, Eigen::Matrix<double, 3, 1> *t2, Eigen::Matrix<double, 3, 1> *t3)
{
...
static int solver_4p3v(const Eigen::Matrix<double, 2, 4>& x1, const Eigen::Matrix<double, 2, 4>& x2, const Eigen::Matrix<double, 2, 4>& x3, ThreeViewsVector* solutions, int iters)
{
...
static void solver_4p3v_para(const std::vector<Eigen::Vector2d> &x1, const std::vector<Eigen::Vector2d> &x2, const std::vector<Eigen::Vector2d> &x3, const std::vector<size_t> &sample, std::vector<ThreeViewCameraPose> *models, int iters, double sq_epipolar_t)
{
    ...
    for (ThreeViews const& sol : solutions)
    {
...
static std::vector<ThreeViewCameraPose> solver_4p3v_para_wrapper(const std::vector<Eigen::Vector2d>& x1, const std::vector<Eigen::Vector2d>& x2, const std::vector<Eigen::Vector2d>& x3, int iters)
{
...
bool solver_tft_4p3vpara(float const* p3d_1, float const* p2d_2, float const* p2d_3, float* r_12, float* t_12, float* r_23, float* t_23, int iterations)
{
    ...
    std::vector<Eigen::Vector2d> x3;

    for (int i = 0; i < 4; ++i)
    {
    x1.push_back(matrix_from_buffer<double, 2, 1>(p3d_1 + (i * 3)));
    x2.push_back(matrix_from_buffer<double, 2, 1>(p2d_2 + (i * 2)));
    x3.push_back(matrix_from_buffer<double, 2, 1>(p2d_3 + (i * 2)));
    }

    std::vector<poselib::ThreeViewCameraPose> result = poselib::solver_4p3v_para_wrapper(x1, x2, x3, iterations);
    ...
    Eigen::Matrix<double, 3, 3> R13;
    Eigen::Matrix<double, 3, 1> t13;

    for (auto const& v : result)
    {
    double s12 = compute_scale(P1(Eigen::indexing::all, Eigen::seqN(0, 4)), p2(Eigen::indexing::all, Eigen::seqN(0, 4)), v.pose12.Rt());

    double error_12 = ((v.pose12.R() * P1.col(4) + s12 * v.pose12.t).colwise().hnormalized() - p2.col(4)).norm();
    double error_13 = ((v.pose13.R() * P1.col(4) + s12 * v.pose13.t).colwise().hnormalized() - p3.col(4)).norm();

    double error = std::max(error_12, error_13);
