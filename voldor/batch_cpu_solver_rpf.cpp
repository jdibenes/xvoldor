
#include "helpers_opencv.h"
#include "batch_cpu_solver.h"
#include "solvers.h"

struct job_inputs
{
	cv::Point3f const* p3d_1;
	cv::Point2f const* p2k_2;
	bool same;
	float cx;
	float cy;
	int solver;
	bool direction;
	float r0;
	int max_iterations;
};

struct job_output
{
	float* poses;
	float* velocities;
	float* focals;
};

static void batch_cpu_solver_rpf(job_descriptor& jd)
{
	job_inputs* ji = static_cast<job_inputs*>(jd.inputs);
	job_output* jo = static_cast<job_output*>(jd.output);

	cv::Point3f p3d_1[7];
	cv::Point2f p2k_2[7];

	float r[3];
	float t[3];
	float dr[3];
	float dt[3];
	float f[2];

	for (int i = jd.start; i < jd.end; ++i)
	{
	int const* p = get_sample_indices(jd, i);

	for (int m = 0; m < jd.sample_size; ++m)
	{
	int im = p[m];

	p3d_1[m] = ji->p3d_1[im];
	p2k_2[m] = ji->p2k_2[im];
	}

	bool ok;

	switch (ji->solver)
	{
	case 0:  ok = solver_rpf_r7pfi(reinterpret_cast<float*>(p3d_1), reinterpret_cast<float*>(p2k_2), ji->cx, ji->cy, ji->direction, ji->r0, r, t, dr, dt, f, ji->max_iterations); break;
	default: ok = false;                                                                                                                                                          break;
	}

	if (!ok) { continue; }

	put_solution_6(jd, jo->poses, r, t);
	if (jo->velocities) { put_solution_6(jd, jo->velocities, dr, dt); }
	if (jo->focals) { put_solution_f(jd, jo->focals, f); }

	jd.valid++;
	}
}

int batch_cpu_solver_rpf(cv::Point3f const* p3d_1, cv::Point2f const* p2k_2, int point_count, cv::Mat const& K, int solver, int poses_to_sample, float* poses, float* velocities, float* focals, int workers, bool unique, bool same, bool direction, float r0, int max_iterations)
{
	job_inputs ji;
	job_output jo;

	ji.p3d_1 = p3d_1;
	ji.p2k_2 = p2k_2;
	ji.same = same;
	ji.cx = K.at<float>(0, 2);
	ji.cy = K.at<float>(1, 2);
	ji.solver = solver;
	ji.direction = direction;
	ji.r0 = r0;
	ji.max_iterations = max_iterations;

	jo.poses = poses;
	jo.velocities = velocities;
	jo.focals = focals;

	int sample_size;

	switch (solver)
	{
	case 0:  sample_size = 7; break;
	default: return 0;
	}

	if (point_count < sample_size) { return 0; }

	std::vector<job_result> jr = batch_solve(poses_to_sample, workers, batch_cpu_solver_rpf, &ji, point_count, sample_size, unique, &jo);
	if (velocities) { batch_finalize(jr, velocities, 6); }
	if (focals) { batch_finalize(jr, focals, 2); }
	return batch_finalize(jr, poses, 6);
}
